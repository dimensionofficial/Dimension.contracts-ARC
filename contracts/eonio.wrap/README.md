...
Execute a transaction while bypassing regular authorization checks (requires authorization of eonio.wrap which needs to be a privileged account).

### eonio.wrap::exec    executer trx
...
## 2. Installing the eonio.wrap contract

The eonio.wrap contract needs to be installed on a privileged account to function. It is recommended to use the account `eonio.wrap`.

First, the account `eonio.wrap` needs to be created. Since it has the restricted `eonio.` prefix, only a privileged account can create this account. So this guide will use the `eonio` account to create the `eonio.wrap` account. On typical live blockchain configurations, the `eonio` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `eonio.msig` contract to help coordinate the approvals of the proposed transaction that creates the `eonio.wrap` account.

The `eonio.wrap` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`eonio`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `eonio` account needs to have enough of the core system token (the `EON` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `eonio` account.

This guide will be using cleos to carry out the process.

### 2.1 Create the eonio.wrap account

#### 2.1.1 Generate the transaction to create the eonio.wrap account

The transaction to create the `eonio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the cleos command to propose the transaction to the eonio.msig contract.

A simple way to generate a transaction to create a new account is to use the `cleos system newaccount`. However, that sub-command currently only accepts a single public key as the owner and active authority of the new account. However, the owner and active authorities of the new account should only be satisfied by the `active` permission of `eonio`. One option is to create the new account with the some newly generated key, and then later update the authorities of the new account using `cleos set account permission`. This guide will take an alternative approach which atomically creates the new account in its proper configuration.

Three unsigned transactions will be generated using cleos and then the actions within those transactions will be appropriately stitched together into a single transaction which will later be proposed using the eonio.msig contract.

First, generate a transaction to capture the necessary actions involved in creating a new account:
```
$ cleos system newaccount -s -j -d --transfer --stake-net "1.000 EON" --stake-cpu "1.000 EON" --buy-ram-kbytes 50 eonio eonio.wrap EOS8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
726964ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea305500c80000"} arg: {"code":"eonio","action":"buyrambytes","args":{"payer":"eonio","receiver":"eonio.wrap","bytes":51200}}
726967ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea3055102700000000000004535953000000001027000000000000045359530000000001"} arg: {"code":"eonio","action":"delegatebw","args":{"from":"eonio","receiver":"eonio.wrap","stake_net_quantity":"1.0000 EON","stake_cpu_quantity":"1.0000 EON","transfer":true}}
...
$ cat newaccount_payload.json
{
   ...
   "owner": {
      ...
      "keys": [],
      "accounts": [{
         "permission": {"actor": "eonio", "permission": "active"},
         "weight": 1
      }],
      "waits": []
   },
...
It will be useful to have a JSON of the active permissions of each of the active block producers for later when proposing transactions using the eonio.msig contract.
...
#### 2.1.2 Propose the transaction to create the eonio.wrap account
...
#    eonio.msig <= eonio.msig::propose          {"proposer":"blkproducera","proposal_name":"createwrap","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.3 Review and approve the transaction to create the eonio.wrap account
...
#    eonio.msig <= eonio.msig::approve          {"proposer":"blkproducera","proposal_name":"createwrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.4 Execute the transaction to create the eonio.wrap account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `eonio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the eonio.msig contract).
...
#    eonio.msig <= eonio.msig::exec             {"proposer":"blkproducera","proposal_name":"createwrap","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the `eonio.wrap` was created:
```
...
net bandwidth:
     staked:          1.0000 EON           (total stake delegated from account to self)
     delegated:       0.0000 EON           (total staked delegated to account from others)
     used:                 0 bytes
     available:        2.304 MiB  
     limit:            2.304 MiB  
...
### 2.2 Deploy the eonio.wrap contract

#### 2.2.1  Generate the transaction to deploy the eonio.wrap contract

The transaction to deploy the contract to the `eonio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the cleos command to propose the transaction to the eonio.msig contract.

The easy way to generate this transaction is using cleos:
```
$ cleos set contract -s -j -d eonio.wrap contracts/eonio.wrap/ > deploy_wrap_contract_trx.json
Reading WAST/WASM from contracts/eonio.wrap/eonio.wrap.wasm...
...
#### 2.2.2 Propose the transaction to deploy the eonio.wrap contract
...
#    eonio.msig <= eonio.msig::propose          {"proposer":"blkproducera","proposal_name":"deploywrap","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.3 Review and approve the transaction to deploy the eonio.wrap contract
...
#    eonio.msig <= eonio.msig::approve          {"proposer":"blkproducera","proposal_name":"deploywrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.4 Execute the transaction to create the eonio.wrap account
...
#    eonio.msig <= eonio.msig::exec             {"proposer":"blkproducera","proposal_name":"deploywrap","executer":"blkproducera"}
```

Anyone can now verify that the `eonio.wrap` contract was deployed correctly.

```
$ cleos get code -a retrieved-eonio.wrap.abi eonio.wrap
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-eonio.wrap.abi
$ sha256sum contracts/eonio.wrap/eonio.wrap.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/eonio.wrap/eonio.wrap.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-eonio.wrap.abi, can then be compared to the original ABI of the contract (contracts/eonio.wrap/eonio.wrap.abi) to ensure they are semantically the same.

## 3. Using the eonio.wrap contract

### 3.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed eonio.wrap contract together with the eonio.msig contract to allow a greater than two-thirds supermajority of block producers of an EOSIO blockchain to change the owner authority of an arbitrary account. The example will use cleos: in particular, the `cleos multisig` command, the `cleos set account permission` sub-command, and the `cleos wrap exec` sub-command. However, the guide also demonstrates what to do if the `cleos wrap exec` sub-command is not available.
...
```
$ cleos set account permission -s -j -d alice owner '{"threshold": 1, "accounts": [{"permission": {"actor": "eonio", "permission": "active"}, "weight": 1}]}' > update_alice_owner_trx.json
```
...
The next step is to generate the transaction containing the `eonio.wrap::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.
...
Then generate the template for the transaction containing the `eonio.wrap::exec` action:
```
$ cleos push action -s -j -d eonio.wrap exec '{"executer": "blkproducera", "trx": ""}' > wrap_update_alice_owner_trx.json
$ cat wrap_update_alice_owner_trx.json
{
...
#    eonio.msig <= eonio.msig::propose          {"proposer":"blkproducera","proposal_name":"updatealice","requested":[{"actor":"blkproducera","permi...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
#    eonio.msig <= eonio.msig::approve          {"proposer":"blkproducera","proposal_name":"updatealice","level":{"actor":"blkproducerb","permission...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
#    eonio.msig <= eonio.msig::exec             {"proposer":"blkproducera","proposal_name":"updatealice","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```
