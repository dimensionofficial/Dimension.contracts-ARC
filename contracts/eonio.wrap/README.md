...
This example will demonstrate how to use the deployed eonio.wrap contract together with the eonio.msig contract to allow a greater than two-thirds supermajority of block producers of an eonio blockchain to change the owner authority of an arbitrary account. The example will use cleos: in particular, the `cleos multisig` command, the `cleos set account permission` sub-command, and the `cleos wrap exec` sub-command. However, the guide also demonstrates what to do if the `cleos wrap exec` sub-command is not available.
