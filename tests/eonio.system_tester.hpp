...
public:
   ...
   void basic_setup() {
      ...
      create_accounts({ N(eonio.token), N(eonio.ram), N(eonio.ramfee), N(eonio.stake),
               N(eonio.bpay), N(eonio.vpay), N(eonio.saving), N(eonio.names), N(eonio.rex) });
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      create_currency( N(eonio.token), config::system_account_name, asset(100000000000000, core_symbol) );
      issue(config::system_account_name, asset(10000000000000, core_symbol) );
      BOOST_REQUIRE_EQUAL( asset(10000000000000, core_symbol), get_balance( "eonio", core_symbol ) );
   }
   ...
   void remaining_setup() {
      ...
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance("eonio")  + get_balance("eonio.ramfee") + get_balance("eonio.stake") + get_balance("eonio.ram") );
   }
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(eonio.token), act, N(accounts), balance_symbol.to_symbol_code().value );
   ...
   abi_serializer initialize_multisig() {
      abi_serializer msig_abi_ser;
      {
         ...
         BOOST_REQUIRE_EQUAL( success(), buyram( "eonio", "eonio.msig", core_sym::from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "eonio", "alice1111111", core_sym::from_string("650000000.0000"), "eonio" );
