...
public:
   eosio_msig_tester() {
      create_accounts( { N(eonio.msig), N(eonio.stake), N(eosio.ram), N(eosio.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, eosio_msig_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"),
                        get_balance("eonio") + get_balance("eosio.ramfee") + get_balance("eonio.stake") + get_balance("eosio.ram") );
