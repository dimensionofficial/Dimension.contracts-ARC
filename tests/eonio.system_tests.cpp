...
BOOST_FIXTURE_TEST_CASE( buysell, eosio_system_tester ) try {
   ...
   transfer( "eonio", "alice1111111", core_sym::from_string("1000.0000"), "eonio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "eonio", "alice1111111", core_sym::from_string("200.0000"), core_sym::from_string("100.0000") ) );

   auto total = get_total_stake( "alice1111111" );
   auto init_bytes =  total["ram_bytes"].as_uint64();

   const asset initial_ram_balance = get_balance(N(eonio.ram));
   const asset initial_ramfee_balance = get_balance(N(eonio.ramfee));
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_sym::from_string("199.0000"), get_balance(N(eonio.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_sym::from_string("1.0000"), get_balance(N(eonio.ramfee)) );
   ...
   transfer( "eonio", "alice1111111", core_sym::from_string("100000000.0000"), "eonio" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, eosio_system_tester ) try {
   ...
   const auto init_eosio_stake_balance = get_balance( N(eonio.stake) );
   ...
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance + core_sym::from_string("300.0000"), get_balance( N(eonio.stake) ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000.0000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance, get_balance( N(eonio.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, eosio_system_tester ) try {
   ...
   issue( "eonio", core_sym::from_string("1000.0000"), config::system_account_name );
   issue( "eonio.stake", core_sym::from_string("1000.0000"), config::system_account_name );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"), get_balance( "alice1111111" ) );

   //eosio stakes for alice with transfer flag

   transfer( "eonio", "bob111111111", core_sym::from_string("1000.0000"), "eonio" );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "eonio", "alice1111111", core_sym::from_string("2000.0000"), core_sym::from_string("1000.0000") ) );
...
BOOST_AUTO_TEST_CASE(votepay_transition2, * boost::unit_test::tolerance(1e-10)) try {
   ...
   std::string old_contract_core_symbol_name = "EON"; // Set to core symbol used in contracts::util::system_wasm_old()
...
BOOST_FIXTURE_TEST_CASE(producer_onblock_check, eosio_system_tester) try {
   ...
   BOOST_REQUIRE_EQUAL(success(), vote( N(producvotera), vector<account_name>(producer_names.begin(), producer_names.begin()+10)));
   BOOST_CHECK_EQUAL( wasm_assert_msg( "cannot undelegate bandwidth until the chain is activated (at least 1 000 000)" ),
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, eosio_system_tester ) try {
   ...
   transfer( "eonio", "alice1111111", core_sym::from_string("600000000.0000"), "eonio" );
...
BOOST_FIXTURE_TEST_CASE( buyname, eosio_system_tester ) try {
   ...
   transfer( "eonio", "nofail", core_sym::from_string( "1000.0000" ) );
...
BOOST_FIXTURE_TEST_CASE( eosioram_ramusage, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( false, get_row_by_account( N(eonio.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );

   //remove row
   base_tester::push_action( N(eonio.token), N(close), N(alice1111111), mvo()
                             ( "owner", "alice1111111" )
                             ( "symbol", symbol{CORE_SYM} )
   );
   BOOST_REQUIRE_EQUAL( true, get_row_by_account( N(eonio.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );

   auto rlm = control->get_resource_limits_manager();
   auto eosioram_ram_usage = rlm.get_account_ram_usage(N(eonio.ram));
   auto alice_ram_usage = rlm.get_account_ram_usage(N(alice1111111));

   BOOST_REQUIRE_EQUAL( success(), sellram( "alice1111111", 2048 ) );

   //make sure that ram was billed to alice, not to eonio.ram
   BOOST_REQUIRE_EQUAL( true, alice_ram_usage < rlm.get_account_ram_usage(N(alice1111111)) );
   BOOST_REQUIRE_EQUAL( eosioram_ram_usage, rlm.get_account_ram_usage(N(eonio.ram)) );
...
BOOST_FIXTURE_TEST_CASE( ram_gift, eosio_system_tester ) try {
   ...
   */
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "eonio", "alice1111111", core_sym::from_string("20.0000"), core_sym::from_string("10.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( unstake_buy_rex, eosio_system_tester, * boost::unit_test::tolerance(1e-10) ) try {
...

   {
      ...
      const asset init_eosio_stake_balance = get_balance( N(eonio.stake) );
...
BOOST_FIXTURE_TEST_CASE( ramfee_namebid_to_rex, eosio_system_tester ) try {
   ...
   asset cur_ramfee_balance = get_balance( N(eonio.ramfee) );
   BOOST_REQUIRE_EQUAL( success(),                      buyram( alice, alice, core_sym::from_string("20.0000") ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(eonio.ramfee) ), core_sym::from_string("0.1000") + cur_ramfee_balance );
...
BOOST_AUTO_TEST_CASE( setabi_bios ) try {
   ...
   t.create_account(N(eonio.token));
   t.set_abi( N(eonio.token), contracts::token_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(eonio.token) );
      _abi_hash abi_hash;
   ...
   t.set_abi( N(eonio.token), contracts::system_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(eonio.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, base_tester::abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( setabi, eosio_system_tester ) try {
   set_abi( N(eonio.token), contracts::token_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(eonio.token) );
      _abi_hash abi_hash;
   ...
   set_abi( N(eonio.token), contracts::system_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(eonio.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( buy_pin_sell_ram, eosio_system_tester ) try {
   ...
   auto total_res = get_total_stake( "eonio" );
